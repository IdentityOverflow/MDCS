"""
Enhanced OpenAI service with session management and cancellation support.

Extends the existing OpenAIService to integrate with ChatSessionManager
...
from .openai_service import OpenAIService as BaseOpenAIService
...
class OpenAIServiceWithCancellation(BaseOpenAIService):
    """
    Enhanced OpenAI service with session management and cancellation support.
    
    Extends the base OpenAIService to integrate with ChatSessionManager,
    ...
    async def send_message_with_session(
        ...
        """
        Send message to OpenAI with session management and cancellation support.
        ...
        try:
            ...
            logger.info(f"OpenAI request cancelled for session {session_id}")
    ...
    async def send_message_stream_with_session(
        ...
        """
        Send streaming message to OpenAI with session management.
        ...
        try:
            ...
            async for chunk in super().send_message_stream(request):
                ...
                if main_task.cancelled():
                    logger.info(f"OpenAI streaming cancelled for session {session_id}")
            ...
            logger.info(f"OpenAI streaming request cancelled for session {session_id}")
            raise
        except Exception as e:
            # Remove session on any error
            self.session_manager.remove_session(session_id)
    ...
    async def _execute_stream_with_session_wrapper(
        ...
        try:
            ...
            logger.info(f"OpenAI streaming task cancelled for session {session_id}")
            raise
        except Exception:
            self.session_manager.remove_session(session_id)
            raise
...
class OpenAIServiceFactory:
    """
    Factory for creating OpenAIService instances with or without cancellation support.
    
    Provides a way to create enhanced or standard OpenAI services based on configuration.
    """
    
    @staticmethod
    def create_service(with_cancellation: bool = True) -> BaseOpenAIService:
        """
        ...
        Returns:
            OpenAIService instance (enhanced or standard)
        """
        if with_cancellation:
            return OpenAIServiceWithCancellation()
        else:
            return BaseOpenAIService()
    
    @staticmethod
    def create_service_with_session_manager(session_manager: ChatSessionManager) -> OpenAIServiceWithCancellation:
        """
        Create enhanced OpenAI service with specific session manager.
        
        Args:
            session_manager: ChatSessionManager instance to use
            
        Returns:
            OpenAIServiceWithCancellation instance
        """
        return OpenAIServiceWithCancellation(session_manager=session_manager)